...
Resumable run statuses are `paused` and `failed`. `failed` represents infrastructure failure after repeated DB/disk errors; the durable crawl state is still the source of truth, so it resumes like `paused`. `completed`, `completed_with_failures`, and `cancelled` remain terminal. A live `running` run is treated as already claimed, which prevents two concurrent `--resume <run-id>` processes from owning the same frontier. After a hard crash (`kill -9`, OOM, machine loss), a run may remain stuck in `running` because `finalizeRun` never ran; in that case resume performs **stale takeover** when `crawl_runs.updated_at` is older than a configured threshold (`RUN_STALE_AFTER_MS`, default 15 minutes; override with `--stale-after-ms`). The takeover is atomic (`UPDATE ... WHERE status = 'running' AND updated_at < cutoff RETURNING`). While a worker pool is alive it actively heartbeats the run row every ~60 seconds (`RunRepository.touchHeartbeat` → `updated_at = now()`), stopped in the same `finally` block before `finalizeRun`, so a live process rejects takeover even during long rate-limit pauses or retry backoffs with no new URLs or persisted bytes. A crashed process cannot heartbeat and becomes stale. Set `RUN_STALE_AFTER_MS` comfortably above the heartbeat interval plus expected transient DB issues. If the row is fresh or another caller wins the CAS, resume fails with `Run is already running or not stale enough to resume`. Too-low thresholds can race a live crawl; prefer conservative values in production. `limit_reached` is resumable only when the caller provides an explicit increased limit override; changing a finite limit to unlimited counts as an increase. For `max_runtime_seconds`, the same explicit value is accepted because runtime is a per-session budget and resume starts a fresh session. Resume first performs an atomic conditional transition to `running` for the previously observed status (or stale `running` takeover); only the winning process clears `finished_at` and then reclaims all `in_progress` rows for that run to `queued`. `done` URLs are never reprocessed.
...
6. Resume: stale or fresh `in_progress` reset, `done` untouched, `queued` processed; paused runs resume with persisted config; stale `running` takeover after hard crash when `updated_at` exceeds threshold.
